...
#그전까지 메인제외 o return
def calc_angle(labels, image_size, vnum = 7):
    H,W = image_size

    labels = labels.reshape(-1, 2)  # if the coord is linear

    #midlines : the middle line per bone
    mid_lines = np.zeros((int(labels.shape[0] / 2), 2))

    for i in range(mid_lines.shape[0]):
        if i % 2 == 0:
            mid_lines[i] = (labels[i * 2] + labels[i * 2 + 2]) / 2.0
        else:
            mid_lines[i] = (labels[i * 2 - 1] + labels[2 * i + 1]) / 2.0
...
if __name__ == '__main__':
    ...
    labels_train = read_labels('./train_labels')
    ...
    for ind, img in enumerate(train_images):
        ...
        H,W,_ = img.shape
        cobb_angles, pos, mid_points, mid_lines, vec_lines = calc_angle(label, image_size=(H,W))
        cobb_angles = np.asarray([cobb_angles[0],cobb_angles[1][0], cobb_angles[2][0]]).flatten()
        ...
        report_list.append(dict(
            ...
            pos1 = pos[0], pos2 = pos[1], pow11 = pos[2], pos22 = pos[3]


        ))
    df = pd.DataFrame(report_list)
    title = 'angle_calc_report'
